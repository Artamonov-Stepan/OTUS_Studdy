...
@pytest.mark.parametrize(
    "side_a, side_b, side_c, expected_area",
    [
        (3, 4, 5, 6.0),
    ],
)
...
@pytest.mark.parametrize(
    "side_a, side_b, side_c, radius, expected_add_area",
    [
        (3, 4, 5, 5, 84.5),
    ],
)
def test_positive_add_area_with_other_figure(
    side_a, side_b, side_c, radius, expected_add_area
):
    t = Triangle(side_a, side_b, side_c)
    c = Circle(radius)
...
@pytest.mark.parametrize("side_a, side_b, side_c, not_a_figure", [
    (3, 4, 5, "Some_str"),
    (3, 4, 5, 10),
    (3, 4, 5, NotAFigure()),
])
def test_add_area_negative(side_a, side_b, side_c, not_a_figure):
    t = Triangle(side_a, side_b, side_c)
